...
<template>
  <div style="width:100%;height: 100%; float: left; position: relative;">
    ...
    <el-form :inline="true" size="small" style="margin-top: 10px;">
      <el-form-item>
        <el-input v-model="queryData.title" clearable type="number" placeholder="请输入通知名称" @input="changeInput" />
      </el-form-item>
      <el-form-item>
        <el-select v-model="queryData.notify_type" clearable filterable placeholder="请选择通知类型">
          <el-option v-for="item in notifyTypeList" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item>
    ...
    <div class="tableContent">
      ...
      >
        ...
        <el-table-column label="通知名称" min-width="120" prop="title" show-overflow-tooltip>
          <template slot-scope="scope">
            {{ scope.row[scope.column.property] }}
          </template>
        </el-table-column>
        <el-table-column label="通知类型" min-width="150" prop="notify_type" show-overflow-tooltip>
          <template slot-scope="scope">
            {{ getLabelByVal(scope.row[scope.column.property], notifyTypeList) || '-' }}
          </template>
        </el-table-column>
        <el-table-column label="通知图标" min-width="120" prop="icon" show-overflow-tooltip>
          <template slot-scope="scope">
            <div v-if="scope.row[scope.column.property]">
              <div v-if="['png','jpg','jpeg','webp'].includes(getImageExtension(scope.row[scope.column.property]))">
                <el-image
                  ...
                  @click.stop="openImageViewFun(scope.row,'icon')"
                />
              </div>
              <div v-else>
      ...
      <div :style="{maxHeight:cliHeight-100+'px'}" class="content">
        ...
        >
          <el-form-item label="通知名称:" prop="title">
            <el-input v-model="addModal.formData.title" placeholder="请输入通知名称" @input="changeInput" />
          </el-form-item>
          <el-form-item label="通知图标:" prop="icon">
            <div v-if="addModal.formData.icon" class="imgBox">
              <div v-if="['png','jpg','jpeg','webp'].includes(getImageExtension(addModal.formData.icon))">
                <el-image :src="addModal.formData.icon" style="width: 120px;height: 120px" />
              </div>
              <div v-else>
                <a :href="addModal.formData.icon" class="aUnderline">文件</a>
              </div>
          ...
          <el-form-item label="通知类型:" prop="notify_type">
            <el-select v-model="addModal.formData.notify_type" clearable filterable placeholder="请选择通知类型">
              <el-option v-for="item in notifyTypeList" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
          <el-form-item label="描述:" prop="desc">
            <el-input v-model="addModal.formData.desc" placeholder="请输入通知名称" @input="changeInput" />
          </el-form-item>
        </el-form>
      </div>
      <div slot="footer">
  ...
  data() {
    return {
      ...
      addModal: {
        ...
        rules: {
          title: [{ required: true, message: '请输入通知名称！', trigger: 'change' }],
          notify_type: [{ required: true, message: '请选择通知类型！', trigger: 'change' }],
          icon: [{ required: true, message: '请上传通知图标！', trigger: 'change' }],
  ...
  methods: {
    ...
    getDataListFun(num) {
      ...
      const params = {
        ...
        notify_type: Number(this.queryData.notify_type),
